...
public interface BankRepository extends JpaRepository<Bank, Long> {
    ...
    @Query(value = "select * from bank where is_active = 1", nativeQuery = true)
    List<Bank> getAllActiveAccounts();

    @Query(value = "select * from bank where id = :bankid and is_active = 1", nativeQuery = true)
    Bank getActiveBankAccount(Long bankid);

    @Query(value = "select * from bank where account_number like %:key% and is_active = 1 " +
